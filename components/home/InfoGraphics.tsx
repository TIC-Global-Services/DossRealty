  ...
  return (
    ...
    >
      ...
      <path
        ...
        strokeMiterlimit={10}
      />
      <path d="M60.6294,31.3637c7.2021-3.6136,14.6283-6.776,22.221-9.4627" stroke="currentColor" strokeWidth={1.12} strokeMiterlimit={10} />
      <path d="M62.497,46.2256c6.6988-2.5815,13.233-5.5945,19.5491-9.0146" stroke="currentColor" strokeWidth={1.12} strokeMiterlimit={10} />
      <path d="M63.0789,61.449c6.6789-2.8427,13.3578-5.6853,20.0366-8.528" stroke="currentColor" strokeWidth={1.12} strokeMiterlimit={10} />
      <path d="M110.9885,74.66c-.8363,7.17-1.1242,14.4045-.8606,21.6187" stroke="currentColor" strokeWidth={1.12} strokeMiterlimit={10} />
      <path d="M136.449,50.56c.3022,9.6486,1.709,27.9175,1.1402,37.5539" stroke="currentColor" strokeWidth={1.12} strokeMiterlimit={10} />
      <path
      ...
      <path
        ...
        strokeMiterlimit={10}
      />
      <path d="M149.0791,17.1583c-.3409,3.6739-.3591,7.378-.0546,11.0551" stroke="currentColor" strokeWidth={1.12} strokeMiterlimit={10} />
      <path d="M155.6208,17.4953c-.2305,3.8247-.2548,7.662-.0728,11.4894" stroke="currentColor" strokeWidth={1.12} strokeMiterlimit={10} />
      <path d="M150.6756,35.8361c-.204,4.3795-.2006,8.7687.0104,13.1478" stroke="currentColor" strokeWidth={1.12} strokeMiterlimit={10} />
      <path d="M155.6983,37.2719c-.0672,3.6535.1425,7.312.6267,10.9335" stroke="currentColor" strokeWidth={1.12} strokeMiterlimit={10} />
      <path d="M33.9952,47.9597c-1.0367,6.7426-1.3563,13.6181-.9502,20.4418" stroke="currentColor" strokeWidth={1.12} strokeMiterlimit={10} />
    </svg>
  );
}
...
export default function InfoGraphics() {
  ...
  useEffect(() => {
    ...
    // ── Desktop: pinned scrub with stroke-draw ──────────────────
    const ctx = gsap.context(() => {
      // prep ALL icons upfront so the scrub is fully bidirectional.
      // Every icon starts hidden (dashoffset = length) + stacked.
      // The scrubbed timeline then draws / undraws – no callbacks, so reverse is exact.
      allDesktop.forEach((el, i) => {
        if (!el) return;
        prepStroke(el);
        // first icon starts visible (its strokes are hidden until the first segment draws)
        // others start invisible underneath – faded in as their draw segment begins
        gsap.set(el, { opacity: i === 0 ? 1 : 0 });
        // filled shapes in 30YEARS start transparent; scrubbed fill tween reveals them
      ...
      // ── first icon draws in the opening beat (so reverse really undraws) ──
      {
        const firstNodes = strokeNodes(allDesktop[0]);
        const firstFills = allDesktop[0]?.querySelectorAll<SVGGeometryElement>("[fill='white']");
        tl.to(
          firstNodes,
          { strokeDashoffset: 0, duration: 0.95, stagger: 0.04, ease: "none" },
          0
        );
        if (firstFills && firstFills.length) {
          tl.to(Array.from(firstFills), { fillOpacity: 1, duration: 0.28, ease: "none" }, 0.62);
        }
      }
      ...
      infographicData.forEach((_, index) => {
        ...
        const prevIcon = desktopIconRefs.current[index - 1];
        const curIcon = desktopIconRefs.current[index];
        ...
        const curNodes = strokeNodes(curIcon);
        const curFills = curIcon
          ? Array.from(curIcon.querySelectorAll<SVGGeometryElement>("[fill='white']"))
          : [];

        tl.to({}, { duration: 0.45 });
        tl.addLabel(`transition-${index}`);

        // fade next icon container in underneath the stroke draw (scrubbed, so it undraws on reverse)
        tl.to(curIcon, { opacity: 1, duration: 0.22, ease: "none" }, "<");

        // ── STROKE DRAW – replaces the old diagonal wipe ────────
        tl.to(
          curNodes,
          {
            strokeDashoffset: 0,
        ...
        if (curFills.length) {
          tl.to(curFills, { fillOpacity: 1, duration: 0.28, ease: "none" }, "<+0.95");
        }
      ...
      <div className="hidden h-full grid-cols-[120px_1fr_320px] items-center px-6 lg:grid lg:px-10">
        ...
        <div className="flex items-center justify-center xl:ml-[18%]">
          <div className="relative h-[clamp(280px,40vw,460px)] w-[clamp(280px,40vw,460px)]">
            {/* soft circular frame */}
            <div className="absolute inset-0 rounded-full border border-black/[0.08] bg-[#FCFCFC]" />
            <div className="absolute inset-[10px] rounded-full border border-black/[0.04]" />
      ...
      <div className="flex h-full flex-col items-center px-6 pb-8 pt-20 lg:hidden">
        <div className="mb-10 mt-10 flex items-center justify-center">
          <div className="relative h-[min(78vw,400px)] w-[min(78vw,400px)]">
            <div className="absolute inset-0 rounded-full border border-black/10 bg-[#FCFCFC]" />
            <div className="absolute inset-[10px] rounded-full border border-black/[0.04]" />
